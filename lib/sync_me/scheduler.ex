defmodule SyncMe.Scheduler do
  ...
  alias SyncMe.Repo
  alias SyncMe.{Events, Events, Availability, Bookings, Partners }
  alias SyncMe.{Availability.AvailabilityRule, Events.EventType, Bookings.Booking, Partners.Partner}
  ...
  def list_bookings_for_date(partner_id, date) do
    ...
    from(b in Booking,
      where: b.partner_id == ^partner_id and
  ...
  def generate_slots_for_rule(rule, date, duration, bookings) do
    start_dt = DateTime.new!(date, rule.start_time, "Etc/UTC")
    end_dt = DateTime.new!(date, rule.end_time, "Etc/UTC")

    # Generate slots in 15-minute increments (adjust as needed)
    Stream.iterate(start_dt, &DateTime.add(&1, 15 * 60, :second))
